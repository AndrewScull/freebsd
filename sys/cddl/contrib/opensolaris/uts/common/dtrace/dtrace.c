...
{
  // We fail if the jit failed so this is just a shell to invoke the jit
  // function which the compiler should optimize.
  return difo->dtdo_jit(difo, mstate, vstate, state,
    ...
    cred_t *cr)
{
	...
	if (dtrace_dif_compile(dp, vstate, &jit_helpers) == NULL) {
		dtrace_difo_release(dp, vstate);
    goto err;
  }
	return (dp);
